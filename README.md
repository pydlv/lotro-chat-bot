...
| `cheatengine-library-master/example/c++/scanner` | C++ source for `scanner.exe`. Scans LOTRO's process memory for the hook address. |
...
2. At startup, `scanner.exe` (built from `cheatengine-library-master/example/c++/scanner`) scans LOTRO's virtual memory for the sentinel and prints the address.
...
- `scanner.exe` compiled from `cheatengine-library-master/example/c++/scanner` and placed at the **repository root**
